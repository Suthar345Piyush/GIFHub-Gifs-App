...
const FilterGif = ({alignLeft = false , showTrending = false}) => {
  ...
  return (
    <div className={`flex my-3 gap-3 ${alignLeft ? "" : "justify-end"} ${
      showTrending ? "flex-col sm:flex-row sm:items-center justify-between"
      : ""
    }`}>
    {showTrending && (
      <span className="flex gap-2">
      { showTrending && (
          <HiMiniArrowTrendingUp size={25} className = "text-teal-700" />
        )}
      <span className ="font-semibold text-white hover:text-yellow-400 cursor-pointer">Trending</span>
      </span>
    )}
